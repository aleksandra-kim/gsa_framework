...
#     except:
#         uncertain_groups = [g for g in lca.technosphere_mm.groups if g.has_distributions]
#         if len(uncertain_groups) > 0:
#             # dtype = uncertain_groups[0].rng.params.dtype
#             # tech_params = np.empty(0, dtype)
#             # for group in uncertain_groups:
#             #     tech_params = np.hstack([tech_params, group.rng.params])
#             dtype = uncertain_groups[0].data.dtype
#             tech_params = np.empty(0, dtype)
#             for group in uncertain_groups:
#                 tech_params = np.hstack([tech_params, group.data])
#
#     # get all edges
...
#         if e["to"] != -1:  # filter out head introduced in graph traversal
#             try:
#                 cols = tech_params["col"]
#                 rows = tech_params["row"]
#             except:
#                 cols =
#             uncertainty_distribution = tech_params[
#                 np.logical_and(
#                     rows == e["from"],
#                     cols == e["to"],
#                 )
