...
def dissimilarity_measure(dict_):
    ...
    num_bins = int(np.round(np.sqrt(iterations_per_parameter))) # other options: Sturges', Rice, Doane's formulas, etc
    ...
    for i in range(num_params):
        y_per_parameter = y[(i+1)*iterations_per_parameter : (i+2)*iterations_per_parameter]
        ...
        for diss_measure_density_str, diss_measure_density_fnc in diss_measures_on_densities.items():
            sa_dict[diss_measure_density_str][i] = diss_measure_density_fnc(y_per_parameter_count_normalized, y_base_count_normalized)
    return sa_dict
